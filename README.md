...
- `subscribers` — one row per Discord channel or DM user, `kind ∈ {guild_channel, dm}`, status, `subscribed_at`, `last_reminded`.
- `seen_alerts` — RSS items already dispatched, deduped by `<guid>`.
...
Volume `ews-data` persists `./data/ews.db` across container restarts.
...
src/
  ...
  discord.ts      # client, commands, DM handler, fan-out
  poller.ts       # RSS fetch + dedup
