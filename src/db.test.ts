...
function subId(db: DB, kind: SubscriberKind, discordId: string): number {
	const sub = db.findSubscriber(kind, discordId);
	assert(sub, `subscriber ${kind}:${discordId} not found`);
	return sub.id;
}
...
describe("DB", () => {
	...
	describe("upsertSubscribed", () => {
		...
		it("reactivates an unsubscribed subscriber and clears last_reminded", () => {
			...
			db.stampReminded(subId(db, "dm", "u1"), "2025-05-01T00:00:00.000Z");
	...
	describe("selectDueForReminder", () => {
		it("includes subscribers ≥ 1 year old never reminded, excludes recent or recently-reminded", () => {
			...
			db.stampReminded(subId(db, "dm", "old-recently-reminded"), "2026-03-01T00:00:00.000Z");
			db.upsertSubscribed({
				kind: "dm",
				discordId: "young",
				guildId: null,
			...
			db.stampReminded(subId(db, "dm", "old-stale-reminder"), "2025-01-01T00:00:00.000Z");
