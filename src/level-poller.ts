...
// Loose schema — we only read `current.{emergencyLevel, alertLevel, zScore, asOf}`.
const Dashboard = z
	.object({
		current: z
			.object({
...
// First observation is recorded silently — startup shouldn't blast every subscriber.
